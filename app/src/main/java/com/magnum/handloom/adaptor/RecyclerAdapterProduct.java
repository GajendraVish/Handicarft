...
public class RecyclerAdapterProduct extends RecyclerView.Adapter<RecyclerAdapterProduct.MyViewHolder> implements Filterable {
    ...
    public void onBindViewHolder(MyViewHolder holder, final int position) {
        ...
        holder.product_name.setText(product.getEcomm_product_name());

        if (Config.ENABLE_DECIMAL_ROUNDING) {
            String price = String.format(Locale.GERMAN, "%1$,.0f", product.getEcomm_product_offer_price());
            holder.product_price.setText(price+" Rs/-");
        } else {
           holder.product_price.setText(product.getEcomm_product_offer_price()+" Rs/-");
        //    holder.product_price.setText(String.valueOf(product.getEcomm_product_price()));
        }
        ...
        Picasso.with(context)
                .load(ApiClient.BASE_URL + product.getEcomm_product_image1())
            ...
            protected FilterResults performFiltering(CharSequence charSequence) {
                ...
                if (charString.isEmpty()) {
                    ...
                    for (Product row : productList) {
                        if (row.getEcomm_product_name().toLowerCase().contains(charString.toLowerCase())) {
